...
if (!empty(isset($_POST['siapsaji_item_validate']))) {
    // Insert the new order item
    $query = mysqli_query($con, "update tb_list_order SET catatan='$catatan', status='2' WHERE id_list_order='$id'");
    if ($query) {
        ...
        alert("Pesanan ' . $menu . ' telah siap disajikan");
        </script>
        ';
    } else {
        $message = '
